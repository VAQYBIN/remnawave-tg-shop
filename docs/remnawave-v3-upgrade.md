...
Пустой результат — всё в порядке. Если строки есть, сверьте одного из них
с панелью: возможно, пользователь заведён там под нестандартным именем и без
`telegramId` — тогда связь нужно восстановить вручную, проставив
`users.panel_user_id`.
