...
- [x] **7.1** Создать `web/routers/admin/broadcast.py`:
...
- [x] **7.2** Механизм: Redis Pub/Sub для передачи рассылки боту
  - Web API публикует в канал `broadcast:request` → JSON {id, text, filters}
  - Бот подписан на канал → выполняет рассылку → обновляет `broadcast:status:{id}` в Redis
  - Web API поллит `broadcast:status:{id}` для статуса
- [x] **7.3** Бот: `bot/services/web_broadcast_service.py` — Redis Pub/Sub listener + startup/shutdown интеграция
- [x] **7.4** Финализировать `GET /api/admin/dashboard`:
...
- [x] **7.5** Создать `frontend/src/pages/admin/BroadcastPage.tsx`:
  - Textarea для сообщения
  - Фильтры получателей (всем / с подпиской / без)
  - Кнопка отправки с подтверждением
  - Прогресс-бар (polling статуса каждые 2 сек)
- [x] **7.6** Финализировать `AdminDashboardPage.tsx`:
  - Ряды StatsCards (сегодня / 7 дней / 30 дней / всего)
  - Предупреждение об истекающих подписках
  - Таблица последних 5 платежей с навигацией к юзеру
  - Визуализация соотношения активных/неактивных подписок
- [x] **7.7** Добавить роут `/admin/broadcast` и пункт sidebar

### Ручная проверка Фазы 7
- [x] Dashboard показывает актуальные метрики из БД (7/30-день, истекающие)
- [x] Таблица последних платежей кликабельна → переход к юзеру
- [x] Рассылка отправляется через Redis → бот получает → доставляет
- [x] Прогресс рассылки отображается в реальном времени
- [x] Фильтры получателей работают (только с подпиской / только без)
...
- [x] **8.1** i18n: добавить все тексты админки в `ru.json` и `en.json`
- [x] **8.2** Mobile-responsive: адаптировать AdminLayout (collapsible sidebar, мобильное меню)
- [x] **8.3** Rate limiting: добавить ограничения на admin-эндпоинты (защита от abuse)
- [x] **8.4** Audit log: логирование действий админа (бан, выдача дней, рассылка, изменение настроек) в `MessageLog`
- [x] **8.5** Error boundaries: обёртка для admin-страниц
- [x] **8.6** Toast уведомления: для всех действий (успех, ошибка)
- [x] **8.7** Оптимизация: debounce поиска, кеширование (staleTime в TanStack Query)
- [x] **8.8** Confirm dialogs: для опасных действий (бан, удаление, restart, рассылка)
- [x] **8.9** Loading states и skeleton screens для таблиц и графиков
- [x] **8.10** Обновить `CLAUDE.md` — добавить документацию по админке

### Ручная проверка Фазы 8
- [x] Все тексты переведены на ru и en
- [x] Админка корректно отображается на мобильных устройствах (hamburger меню)
- [x] Действия админа логируются в БД (таблица message_logs)
- [x] Опасные действия требуют подтверждения (ban, delete, restart, broadcast)
- [x] Ошибки API отображаются через toast (ban, unban, add-days, add-traffic, промокоды)
- [x] Поиск не спамит запросами (debounce 300ms работает)
