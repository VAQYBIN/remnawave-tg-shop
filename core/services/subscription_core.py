...
async def get_active_subscription(session: AsyncSession, user_id: int):
    ...
    try:
        ...
        if not user_record:
            return None
        panel_uuid = user_record.panel_user_uuid
...
async def get_user_panel_uuid(session: AsyncSession, user_id: int) -> Optional[str]:
    """Return panel_user_uuid for a given local user_id, or None if not set."""
    user_record = await user_dal.get_user_by_id(session, user_id)
    if user_record:
        return user_record.panel_user_uuid
    return None
